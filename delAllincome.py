...
class deleteAI:
	...
	def confirm(self):
		...
		global e1
		choice=StringVar()
		label1=Label(self.root,text="Do you want to delete all income (Y/N) :",font=('Arial',12,'bold')).place(x=50,y=150)
		e1=Entry(self.root,textvariable=choice).place(x=400,y=150)
		button1=Button(self.root,text="Delete",width=30,command=self.removeAllincome).place(x=550,y=150)

	def removeAllincome(self):
		ch=choice.get()
		if ch=='Y' or ch=='y':
			...
			self.message("All Income deleted successfully")
			self.root.destroy()
		elif (ch=='N' or ch=='n'):
